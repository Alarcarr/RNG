...
final class PlanetNodeScript {
	...
	void render(Node& node) {
		if(gfxFlags & PGA_Ringworld != 0) {
			...
			material::VolkurTrivial.switchTo();
...
//////////////////////////////
			...
			material::RngInner.switchTo();
			model::RingworldInner.draw(lodDist);
			
			material::RngRing.switchTo();
			model::Ring.draw(lodDist);
		
		
			
			//Dibujar 6 tanques
				//GGLUCAS said applyAbsTransform(vec3d(translate.x, translate.y, translate.z), vec3d(scale.x, scale.y, scale.z), quaterniond() for rotation)
			//applyAbsTransform(vec3d() [no translation], vec3d(1.0) [no scale change], quaterniond_fromAxisAngle(vec3d(1.0, 0.0, 0.0), pi) [rotate pi radians across the x axis])
			material::RngFtank.switchTo();
			
			model::ITank.draw(lodDist);
				applyAbsTransform(vec3d(), vec3d(1.0), quaterniond_fromAxisAngle(vec3d(0.0, 1.0, 0.0), pi/3));
			...
			material::RingworldSurface.switchTo();
